...
def make_anfis(x, device, num_mfs=5, num_out=1, hybrid=True, mfs_type: MfsType = MfsType.Gauss):
    '''
    ...
    for i in range(num_invars):
        if mfs_type == MfsType.Sigmoid:
            alpha = 10 / ranges[i]
            clist = torch.linspace(minvals[i], maxvals[i], num_mfs).tolist()
            invars.append(('x{}'.format(i), make_sigmoid_mfs(alpha, clist)))

        elif mfs_type == MfsType.Bell:
            a = ranges[i] / num_mfs
            b = 2  # Slope control (arbitrary default)
            clist = torch.linspace(minvals[i], maxvals[i], num_mfs).tolist()
            invars.append(('x{}'.format(i), make_bell_mfs(a, b, clist)))

        elif mfs_type == MfsType.Triangular:
            width = ranges[i] / num_mfs
            clist = torch.linspace(minvals[i], maxvals[i], num_mfs).tolist()
            invars.append(('x{}'.format(i), make_tri_mfs(width, clist)))

        elif mfs_type == MfsType.Trapezoid:
            slope = ranges[i] / (2 * num_mfs)  # Example slope
            width = ranges[i] / num_mfs
            clist = torch.linspace(minvals[i], maxvals[i], num_mfs).tolist()
            invars.append(('x{}'.format(i), make_trap_mfs(width, slope, clist)))

        elif mfs_type == MfsType.DSigmoid:
            alpha = 10 / ranges[i]  # Control the steepness of the sigmoids
            beta = alpha
            clist1 = torch.linspace(minvals[i], maxvals[i] - ranges[i] / num_mfs, num_mfs).tolist()
            clist2 = torch.linspace(minvals[i] + ranges[i] / num_mfs, maxvals[i], num_mfs).tolist()
            invars.append(('x{}'.format(i), make_dsig_mfs(alpha, beta, clist1, clist2)))
